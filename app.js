...
async function fetchApiKey() {
  ...
  if (!response.ok) {
    ...
    const err = new Error(body.error || `Key request failed (${response.status})`);
    err.isKeyFetchError = true; // reliable marker — don't guess from message text
    throw err;
  }
...
async function startCall() {
  try {
    ...
    console.error(err);
    const message = err && err.isKeyFetchError

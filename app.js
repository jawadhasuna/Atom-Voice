...
// ---- State ----
let ephemeralToken = null; // fetched fresh from our server for each call
let ws = null;
...
// ============================================================
// Fetches a fresh, short-lived token from our own server before each call.
// Our server holds the real API key (via GEMINI_API_KEY env var) and never
// sends it to the browser — only this disposable token.
...
function reactivityLoop(timestamp) {
  ...
  if (orbInner) {
    ...
    const blurAmount = 14 - smoothed * 5; // sharpens slightly as it gets more energetic
    orbInner.style.filter = `blur(${blurAmount.toFixed(1)}px) saturate(1.4)`;
  }
...
function connectWebSocket() {
  setupReady = false;
  // Ephemeral tokens require the v1alpha endpoint and go in access_token,
  // not the key= parameter used for raw API keys.
  const url = `wss://generativelanguage.googleapis.com/ws/google.ai.generativelanguage.v1alpha.GenerativeService.BidiGenerateContent?access_token=${ephemeralToken}`;
  ...
  ws.onopen = () => {
    ...
    hint.textContent = "Finishing setup...";
  };
  ...
  ws.onmessage = async (event) => {
    ...
    if (response.setupComplete) {
      ...
      setStatus("live", true);
      setOrbState("listening");
      hint.textContent = "Listening... just start talking";
      return;
    }
  ...
  try {
    ...
    showError(
      err.message && err.message.includes("Token")
        ? `Couldn't start session: ${err.message}`
...
function endCall() {
  ...
  setOrbState("idle");
  setStatus("not connected");
  hint.textContent = "Tap the circle to start talking";
